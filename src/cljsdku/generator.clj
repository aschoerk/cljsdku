      ...
      (if (>= index (count places))
        (list a level)
	      (let [pair (places index) 
	        ...
	        (if (and (= 1 (count (res :results))) (<= level (eval-way (res :ways)))) ; (res :maxlevel) (res :level))))
             (do 
               ;(println  "way" (res :ways))
               ;(println (thread-name) "in level" level "->" (res :level) "improved to " testarray  )
	             (recur (inc index) testarray (eval-way (res :ways)))) ; (* (res :maxlevel) (res :level))))
